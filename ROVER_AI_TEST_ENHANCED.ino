...
void addToDisplay(String text) {
  ...
  displayLines[MAX_DISPLAY_LINES - 1] = text;
}
...
void updateOLEDDisplay() {
  ...
  // Draw text area (lower portion of screen)
  display.setTextSize(1);
  display.setTextColor(SSD1306_WHITE);
  
  for (int i = 0; i < MAX_DISPLAY_LINES; i++) {
    display.setCursor(0, 48 + (i * 10));
...
void loop() {
  ...
  if (Serial1.available() > 0) {
    ...
    if (aiBuffer.length() > 0) {
      // Display AI command on OLED
      addToDisplay("AI: " + aiBuffer);
      
      if (aiBuffer.indexOf("forward") >= 0 || aiBuffer.indexOf("go") >= 0) {
        roboEyes.setMood(HAPPY);
        motorWifi(1); 
        addToDisplay(">> Moving Forward");
      }
      else if (aiBuffer.indexOf("dance") >= 0 || 
               aiBuffer.indexOf("party") >= 0) {
        roboEyes.setMood(HAPPY);
        addToDisplay(">> Dancing!");
        MOTOR(3, 15, 10, 5);
        MOTOR(4, 15, 10, 5);
        motorWifi(0);
      }
      else if (aiBuffer.indexOf("stop") >= 0 || aiBuffer.indexOf("halt") >= 0) {
        roboEyes.setMood(DEFAULT);
        motorWifi(0);
        addToDisplay(">> Stopped");
      }
      else {
        addToDisplay(">> Unknown Cmd");
      }
  ...
  if (millis() - lastLaserCheck > 300) {
    ...
    distanceMM = laserSensor.readRangeContinuousMillimeters();
    ...
    if (!laserSensor.timeoutOccurred()) {
      if (distanceMM < 150 && distanceMM > 30) {
        ...
        addToDisplay("OBSTACLE: " + String(distanceMM) + "mm");
